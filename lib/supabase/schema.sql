...
-- For example, creating a default user or test data

COMMENT ON TABLE users IS 'Stores user account information and preferences';
COMMENT ON TABLE queries IS 'Stores all search queries made by users';
COMMENT ON TABLE search_results IS 'Stores individual search results for each query';
COMMENT ON TABLE user_sessions IS 'Tracks user sessions and activity';
COMMENT ON TABLE query_feedback IS 'Stores user feedback on query results';
COMMENT ON TABLE api_usage IS 'Tracks API usage and costs';
COMMENT ON TABLE saved_searches IS 'Allows users to save and bookmark searches';
